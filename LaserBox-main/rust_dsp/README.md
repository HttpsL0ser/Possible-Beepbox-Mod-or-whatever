...
Hi! You're looking at the DSP code. Normally, the precompiled WebAssembly module is checked into the repository, so you don't actually need to build this to compile LaserBox.
...
```
$ pwd
<something here>/LaserBox/rust_dsp
```

Install Rust using [rustup](https://rustup.rs/). Follow the instructions; once you're done, execute `rustup` in a terminal (you should be using the same terminal you use to compile LaserBox) to check that it's installed:
...
And you're done! The newly-built module should be automatically picked up when you build LaserBox.
